...
export const HomePageClient = ({ events, news, jobs, products }: HomePageClientProps) => {
  ...
  const heroStats = useMemo(() => {
    return [
      { label: "Campuser", value: campuses.length || "—" },
      { label: "Kommende events", value: eventsForCampus.length || "—" },
      { label: "Ledige verv", value: jobsForCampus.length || "—" },
    ];
  ...
  const heroSubtitle = activeCampus
    ? `Opplev alt som skjer ved ${activeCampus.name}. Finn arrangementer, fordeler og verv som gjør studietiden uforglemmelig.`
    : "BI Student Organisation samler studenter på tvers av campus. Utforsk mulighetene og finn din vei inn i fellesskapet.";
  ...
  return (
    <div className="space-y-20 text-primary-100">
        ...
        <div className="relative grid gap-10 px-6 py-12 md:px-10 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-center lg:py-16 xl:px-14">
          <div className="space-y-6">
            <h1 className="max-w-2xl text-3xl font-semibold leading-tight text-white drop-shadow md:text-5xl">
              {activeCampus ? `Bli med i pulsen ved ${activeCampus.name}` : "En sterk studentstemme på alle BI-campuser"}
            </h1>
          ...
          <div className="relative hidden h-full min-h-[320px] rounded-3xl border border-white/20 bg-white/10 shadow-glow lg:flex lg:flex-col lg:justify-between">
            <div className="relative flex-1 overflow-hidden rounded-3xl">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 text-sm">
                <p className="text-white/90">Mer enn studentliv</p>
                <p className="text-lg font-semibold text-white">Skap opplevelser, påvirk politikk og bygg nettverk for fremtiden.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          {
            title: "Finn din enhet",
            description: "Markedsføring, HR, økonomi eller event – se hvor du kan bidra.",
            href: "/units",
            cta: "Utforsk enheter",
          },
          {
            title: "Kommende høydepunkter",
            description: "Få oversikt over fester, faglige møteplasser og karrieredager.",
            href: "/events",
            cta: "Se kalender",
          },
          {
            title: "Søk verv",
            description: "Utvikle deg i studentpolitikk, ledelse eller koordinering.",
            href: "/jobs",
            cta: "Åpne verv",
          },
          {
            title: "Studentfordeler",
            description: "Shop, rabatter og ressurser eksklusivt for BISO-medlemmer.",
            href: "/shop",
            cta: "Til fordeler",
          },
        ].map((item) => (
      ...
      <section className="space-y-8">
        <SectionHeader
          title={activeCampus ? `Aktuelt fra ${activeCampus.name}` : "Aktuelt i BISO"}
          description="Nyheter, kunngjøringer og historier fra hele organisasjonen."
          href="/news"
      ...
      <section className="space-y-8">
        <SectionHeader
          title="Eventkalender"
          description={activeCampus ? `Planlegg uka ved ${activeCampus.name}.` : "Se hva som skjer på dine campuser."}
          href="/events"
        ...
        {spotlightEvents.length ? (
                  ...
                  <CardContent className="space-y-3 p-6">
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <span className="rounded-full bg-secondary-10 px-2 py-1 text-secondary-100">
                        {campusLookup[event.campus] ?? "BISO"}
                      </span>
                    </div>
                    ...
                    <Button asChild variant="outline" size="sm" className="w-full">
                      <Link href={`/events/${event.$id}`}>Se detaljer</Link>
                    </Button>
                  </CardContent>
                </Card>
              ))}
          ...
          <div className="rounded-2xl border border-dashed border-primary/20 bg-white/60 p-10 text-center text-muted-foreground">
            Ingen kommende arrangementer for denne campusen.
          </div>
        )}
      </section>
      ...
      <section className="space-y-8">
        <SectionHeader
          title="Ledige verv og jobber"
          description="Bli med i organisasjonen og form fremtidens studentliv."
          href="/jobs"
          linkLabel="Se alle stillinger"
        />
        <div className="grid gap-6 lg:grid-cols-3">
              ...
              <Card key={job.$id} className="flex h-full flex-col justify-between border border-primary/10 bg-white/90 shadow-md transition hover:-translate-y-1 hover:shadow-card-hover">
                <CardContent className="space-y-4 p-6">
                  <div className="flex items-center justify-between text-xs font-medium text-muted-foreground">
                    <span className="rounded-full bg-primary/10 px-2 py-1 text-primary-30">
                      {campusLookup[job.campus] ?? "BISO"}
                    </span>
                    <span>
                      {job.application_deadline ? `Frist: ${formatDateReadable(job.application_deadline)}` : "Løpende opptak"}
                    </span>
                  </div>
                ...
                <div className="px-6 pb-6">
                  <Button asChild variant="outline" className="w-full">
                    <Link href={`/jobs/${job.slug}`}>Les mer</Link>
                  </Button>
                </div>
              </Card>
            ))
      ...
      <section className="space-y-8">
        <SectionHeader
          title="Shop: nye favoritter"
          description="Profiler din campus med eksklusiv BISO-merch."
          href="/shop"
          linkLabel="Gå til butikk"
        />
              ...
              <Card key={product.$id} className="group overflow-hidden border border-primary/10 bg-white/90 shadow-md transition hover:-translate-y-1 hover:shadow-card-hover">
                ...
                <CardContent className="space-y-2 p-6">
                  ...
                  {typeof product.price === "number" ? (
                    <div className="text-base font-semibold text-primary-40">{product.price.toFixed(2)} kr</div>
                  ) : null}
                </CardContent>
              </Card>
            ))
